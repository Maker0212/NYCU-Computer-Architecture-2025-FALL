...
`define LOG_S 4
        ...
        // `define SLOTS  2   `define LOG_S  1
        // `define SLOTS  3   `define LOG_S  2
        // `define SLOTS  4   `define LOG_S  2
        // `define SLOTS  5   `define LOG_S  3
        // `define SLOTS  6   `define LOG_S  3
        // `define SLOTS  7   `define LOG_S  3
        // `define SLOTS  8   `define LOG_S  3
        // `define SLOTS  9   `define LOG_S  4
        // `define SLOTS 10   `define LOG_S  4
        // `define SLOTS 11   `define LOG_S  4
        // `define SLOTS 12   `define LOG_S  4
        // `define SLOTS 13   `define LOG_S  4
        // `define SLOTS 14   `define LOG_S  4
        // `define SLOTS 15   `define LOG_S  4
        // `define SLOTS 16   `define LOG_S  4
...
`define RISCV_INST_MSG_SHAMT    24:20
        ...
        // Immediate field extraction ranges
                    ...
                    begin
                      ...
                      casez ( msg )
                        ...
                        `RISCV_INST_MSG_NOP     :
                          $sformat( dasm, "nop                      "                         );
                        `RISCV_INST_MSG_LUI     :
                          $sformat( dasm, "lui    r%02d, %08x     ",         rd,       imm_u  );
                        `RISCV_INST_MSG_AUIPC   :
                          $sformat( dasm, "auipc  r%02d, %08x     ",         rd,       imm_u  );
                        `RISCV_INST_MSG_JAL     :
                          $sformat( dasm, "jal    r%02d, %08x     ",         rd,       imm_uj );
                        `RISCV_INST_MSG_JALR    :
                          $sformat( dasm, "jalr   r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_BEQ     :
                          $sformat( dasm, "beq    r%02d, r%02d, %08x",       rs1, rs2, imm_sb );
                        `RISCV_INST_MSG_BNE     :
                          $sformat( dasm, "bne    r%02d, r%02d, %08x",       rs1, rs2, imm_sb );
                        `RISCV_INST_MSG_BLT     :
                          $sformat( dasm, "blt    r%02d, r%02d, %08x",       rs1, rs2, imm_sb );
                        `RISCV_INST_MSG_BGE     :
                          $sformat( dasm, "bge    r%02d, r%02d, %08x",       rs1, rs2, imm_sb );
                        `RISCV_INST_MSG_BLTU    :
                          $sformat( dasm, "bltu   r%02d, r%02d, %08x",       rs1, rs2, imm_sb );
                        `RISCV_INST_MSG_BGEU    :
                          $sformat( dasm, "bgeu   r%02d, r%02d, %08x",       rs1, rs2, imm_sb );
                        `RISCV_INST_MSG_LB      :
                          $sformat( dasm, "lb     r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_LH      :
                          $sformat( dasm, "lh     r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_LW      :
                          $sformat( dasm, "lw     r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_LBU     :
                          $sformat( dasm, "lbu    r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_LHU     :
                          $sformat( dasm, "lhu    r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_SB      :
                          $sformat( dasm, "sb     r%02d, r%02d, %08x",       rs2, rs1, imm_s  );
                        `RISCV_INST_MSG_SH      :
                          $sformat( dasm, "sh     r%02d, r%02d, %08x",       rs2, rs1, imm_s  );
                        `RISCV_INST_MSG_SW      :
                          $sformat( dasm, "sw     r%02d, r%02d, %08x",       rs2, rs1, imm_s  );
                        `RISCV_INST_MSG_ADDI    :
                          $sformat( dasm, "addi   r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_SLTI    :
                          $sformat( dasm, "slti   r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_SLTIU   :
                          $sformat( dasm, "sltiu  r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_ANDI    :
                          $sformat( dasm, "andi   r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_ORI     :
                          $sformat( dasm, "ori    r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_XORI    :
                          $sformat( dasm, "xori   r%02d, r%02d, %08x",       rd,  rs1, imm_i  );
                        `RISCV_INST_MSG_SLLI    :
                          $sformat( dasm, "slli   r%02d, r%02d, %08x",       rd,  rs1, shamt  );
                        `RISCV_INST_MSG_SRLI    :
                          $sformat( dasm, "srli   r%02d, r%02d, %08x",       rd,  rs1, shamt  );
                        `RISCV_INST_MSG_SRAI    :
                          $sformat( dasm, "srai   r%02d, r%02d, %08x",       rd,  rs1, shamt  );
                        `RISCV_INST_MSG_ADD     :
                          $sformat( dasm, "add    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_SUB     :
                          $sformat( dasm, "sub    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_SLL     :
                          $sformat( dasm, "sll    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_SLT     :
                          $sformat( dasm, "slt    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_SLTU    :
                          $sformat( dasm, "sltu   r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_XOR     :
                          $sformat( dasm, "xor    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_SRL     :
                          $sformat( dasm, "srl    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_SRA     :
                          $sformat( dasm, "sra    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_OR      :
                          $sformat( dasm, "or     r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_AND     :
                          $sformat( dasm, "and    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_CSRW    :
                          $sformat( dasm, "csrw   r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        // RISCV32M
                        `RISCV_INST_MSG_MUL     :
                          $sformat( dasm, "mul    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_MULH    :
                          $sformat( dasm, "mulh   r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_MULHSU  :
                          $sformat( dasm, "mulhsu r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_MULHU   :
                          $sformat( dasm, "mulhu  r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_DIV     :
                          $sformat( dasm, "div    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_DIVU    :
                          $sformat( dasm, "divu   r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_REM     :
                          $sformat( dasm, "rem    r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        `RISCV_INST_MSG_REMU    :
                          $sformat( dasm, "remu   r%02d, r%02d, r%02d     ", rd,  rs1, rs2    );
                        default                :
                          $sformat( dasm, "undefined inst       " );
                      endcase
                  ...
                  begin
                    ...
                    begin
                      ...
                      casez ( msg )
                        // RISCV32I
                        `RISCV_INST_MSG_NOP     :
                          $sformat( minidasm, "nop   ");
                        `RISCV_INST_MSG_LUI     :
                          $sformat( minidasm, "lui   ");
                        `RISCV_INST_MSG_AUIPC   :
                          $sformat( minidasm, "auipc ");
                        `RISCV_INST_MSG_JAL     :
                          $sformat( minidasm, "jal   ");
                        `RISCV_INST_MSG_JALR    :
                          $sformat( minidasm, "jalr  ");
                        `RISCV_INST_MSG_BEQ     :
                          $sformat( minidasm, "beq   ");
                        `RISCV_INST_MSG_BNE     :
                          $sformat( minidasm, "bne   ");
                        `RISCV_INST_MSG_BLT     :
                          $sformat( minidasm, "blt   ");
                        `RISCV_INST_MSG_BGE     :
                          $sformat( minidasm, "bge   ");
                        `RISCV_INST_MSG_BLTU    :
                          $sformat( minidasm, "bltu  ");
                        `RISCV_INST_MSG_BGEU    :
                          $sformat( minidasm, "bgeu  ");
                        `RISCV_INST_MSG_LB      :
                          $sformat( minidasm, "lb    ");
                        `RISCV_INST_MSG_LH      :
                          $sformat( minidasm, "lh    ");
                        `RISCV_INST_MSG_LW      :
                          $sformat( minidasm, "lw    ");
                        `RISCV_INST_MSG_LBU     :
                          $sformat( minidasm, "lbu   ");
                        `RISCV_INST_MSG_LHU     :
                          $sformat( minidasm, "lhu   ");
                        `RISCV_INST_MSG_SB      :
                          $sformat( minidasm, "sb    ");
                        `RISCV_INST_MSG_SH      :
                          $sformat( minidasm, "sh    ");
                        `RISCV_INST_MSG_SW      :
                          $sformat( minidasm, "sw    ");
                        `RISCV_INST_MSG_ADDI    :
                          $sformat( minidasm, "addi  ");
                        `RISCV_INST_MSG_SLTI    :
                          $sformat( minidasm, "slti  ");
                        `RISCV_INST_MSG_SLTIU   :
                          $sformat( minidasm, "sltiu ");
                        `RISCV_INST_MSG_ANDI    :
                          $sformat( minidasm, "andi  ");
                        `RISCV_INST_MSG_ORI     :
                          $sformat( minidasm, "ori   ");
                        `RISCV_INST_MSG_XORI    :
                          $sformat( minidasm, "xori  ");
                        `RISCV_INST_MSG_SLLI    :
                          $sformat( minidasm, "slli  ");
                        `RISCV_INST_MSG_SRLI    :
                          $sformat( minidasm, "srli  ");
                        `RISCV_INST_MSG_SRAI    :
                          $sformat( minidasm, "srai  ");
                        `RISCV_INST_MSG_ADD     :
                          $sformat( minidasm, "add   ");
                        `RISCV_INST_MSG_SUB     :
                          $sformat( minidasm, "sub   ");
                        `RISCV_INST_MSG_SLL     :
                          $sformat( minidasm, "sll   ");
                        `RISCV_INST_MSG_SLT     :
                          $sformat( minidasm, "slt   ");
                        `RISCV_INST_MSG_SLTU    :
                          $sformat( minidasm, "sltu  ");
                        `RISCV_INST_MSG_XOR     :
                          $sformat( minidasm, "xor   ");
                        `RISCV_INST_MSG_SRL     :
                          $sformat( minidasm, "srl   ");
                        `RISCV_INST_MSG_SRA     :
                          $sformat( minidasm, "sra   ");
                        `RISCV_INST_MSG_OR      :
                          $sformat( minidasm, "or    ");
                        `RISCV_INST_MSG_AND     :
                          $sformat( minidasm, "and   ");
                        `RISCV_INST_MSG_CSRW    :
                          $sformat( minidasm, "csrw  ");
                        // RISCV32M
                        `RISCV_INST_MSG_MUL     :
                          $sformat( minidasm, "mul   ");
                        `RISCV_INST_MSG_MULH    :
                          $sformat( minidasm, "mulh  ");
                        `RISCV_INST_MSG_MULHSU  :
                          $sformat( minidasm, "mulhsu");
                        `RISCV_INST_MSG_MULHU   :
                          $sformat( minidasm, "mulhu ");
                        `RISCV_INST_MSG_DIV     :
                          $sformat( minidasm, "div   ");
                        `RISCV_INST_MSG_DIVU    :
                          $sformat( minidasm, "divu  ");
                        `RISCV_INST_MSG_REM     :
                          $sformat( minidasm, "rem   ");
                        `RISCV_INST_MSG_REMU    :
                          $sformat( minidasm, "remu  ");
                        default                :
                          $sformat( minidasm,  "undef ");
                      endcase
